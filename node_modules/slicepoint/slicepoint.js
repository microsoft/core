...
var util = require('util'),stream = require('stream'),zeroBuffer = new Buffer(0);
...
Slicepoint.prototype._transform = function (chunk, encodeing, done) {
  ...
  var output = Buffer.concat([this._tailBuffer, chunk]),headSliceStart = 0,headSliceEnd = output.length;
  if (this._sliceHead) {
...
Slicepoint.prototype._flush = function (done) {
  ...
  if (this._neverOutput){return done(null);}
  // counting form the end of the stream
  if (this._sliceTail) {
    // - A part of the head should be removed
    ...
    if (this._end < 0){tailSliceEnd = (this._consumed + this._end) - this._tailPosition;}
    // Is begin after end?
    if (this._end > 0) { tailSliceEnd = this._end - this._tailPosition;}
