...
var http = require('http'),
    ...
    url = require('url'),
    mockney = require('mockney'),
    headerP3P = 'policyref="http://info.yahoo.com/w3c/p3p.xml", ' +
...
function FinanceServer() {
  ...
  this.server.on('close', function (){mockney.restore('ichart.finance.yahoo.com:80');});
...
function createDate(year, day, month) { return new Date(Date.UTC(year, month, day))}
FinanceServer.prototype._request = function (req, res) {
  ...
  var from = createDate(href.query.c, href.query.b, href.query.a),
      parsedTo = createDate(href.query.f, href.query.e, href.query.d),
      to = parsedTo;
  if (this.endDate && to.getTime() > this.endDate.getTime()) {to = this.endDate;}
  // Go from `from` to `to`
...
FinanceServer.prototype.stop = function (symbol, date) {this.endDate = date;};
FinanceServer.prototype.getRawRow = function (date) {
  var row = rowSample[date.getDay()];
  if (row === null) return null;
  if (this.endDate && date.getTime() > this.endDate.getTime()) { return null; }
...
FinanceServer.prototype.getParsedRows = function (begin, days, first) {
  var current = new Date(begin.getTime()), rows = [];
...
FinanceServer.prototype.listen = function () {return this.server.listen.apply(this.server, arguments);};
FinanceServer.prototype.close = function () {return this.server.close.apply(this.server, arguments);};
