...
var startDate = offsetDate(today(), -11),stopDate = offsetDate(startDate, 20);
function collectRows(stream, total, callback) {
  ...
  (function flow() {
    ...
    if (rows.length === total) { return callback(null, rows); }
    if (row === null) {
      stream.once('readable', function () {   flow();});
    }
  })();
}
...
finance.listen(0, 'localhost', function () {
  test('general test the flow of the qoutes stream', function (t) {
    var stream,lastDate = startDate,expectedRows,first = true,nextLive = false,lastRequest = Date.now();
      ...
      async.series([
        function (done) {
          finance.once('response', function (symbol, begin, end) {
            ...
            if (!nextLive & end.getTime() >= Date.now()) { nextLive =  true; }
        ...
        function (done) {
          ...
          if (first === false) {expectedRows = expectedRows.slice(1);}
          if (expectedRows.length === 0) {return done(null);}
