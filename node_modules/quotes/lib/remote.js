...
var util = require('util'),stream = require('stream'),newlinepoint = require('newlinepoint'),
    reversepoint = require('reversepoint'),slicepoint = require('slicepoint'),
    csv = require('rfc-csv'),http = require('http'),url = require('url');// Transform stream there converts row arrays intro object with correct type
function Remote(qoutes, begin, end) {
  ...
  if (this.qoutes._live === false) {  this._pull();}
  // Otherwice wait until the timeout has elapsed and then start pulling data
  else {    setTimeout(this._pull.bind(this), this.settings.timeout);}
}
...
Remote.prototype._transform = function (row, encodeing, done) {
  ...
  if (this.qoutes._first === false && thisDate.getTime() <= this.begin.getTime()){ return done(null);}
  this.push({
    date: thisDate,
    open: Number(row[1]),
    high: Number(row[2]),
...
Remote.prototype._flush = function (done) {done(null);};
