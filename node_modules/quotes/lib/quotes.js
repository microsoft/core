...
function Quotes(symbol, since, options) {
  ...
  if (typeof symbol !== 'string') {    throw new TypeError('Company symbol must be a string');}
  if (!(since instanceof Date)) {    throw new TypeError('Start sate mush be a Date object');}
  
  // Default options to empty object
  if (options === undefined) options = {};

  if (isObject(options) === false) {    throw new TypeError('options must be an object');}
...
Quotes.prototype._remote = function (first) {
  ...
  var begin = this._lastDate,end = offsetDate(this._lastDate, this._settings.interval);
  ...
  this._fetch.once('end', function () {
    ...
    if (!self._live && end.getTime() >= Date.now()) {   self._live = true;    }
